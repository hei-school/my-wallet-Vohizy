...
function prendreElements(wallet: any) {
...
function prendreElement(type: string, wallet: any) {
...
function prendreArgent() {
  ...
  try {
    if (parseFloat(montant) > walletModule.argent) {
      throw new Error("Fonds insuffisants dans le wallet.");
    }
    walletModule.argent -= parseFloat(montant);
    console.log(`Vous avez pris ${montant} euros.`);
  } catch (error) {
    console.log(`Erreur : ${error.message}`);
  }
  prendreElements(walletModule.getWallet());
}

export { prendreElements, prendreElement, prendreArgent };
